...
python3 tool_check.py --profile core
python3 triage.py unknown_file
```

Check only the dependencies you need:

```bash
python3 tool_check.py --profile hash
python3 tool_check.py --profile stego
python3 tool_check.py --profile archive
python3 tool_check.py --profile pcap
python3 tool_check.py --profile audio
python3 tool_check.py --profile disk
python3 tool_check.py --profile database
```
...
```bash
# Raw hashes
python3 hash_wrapper.py 482c811da5d5b4bc6d497ffa98491e38 --john
python3 hash_wrapper.py b7a875fc1ea228b9061041b7cec4bd3c52ab3ce3 --john
python3 hash_wrapper.py 916e8c4f79b25028c9e467f1eb8eee6d6bbdff965f9928310ad30a8d88697745 --john

# Existing hash file (the original one-command form)
python3 hash_wrapper.py temp.txt --john -w /usr/share/wordlists/rockyou.txt

# Explicit source forms when path/value intent must be unambiguous
python3 hash_wrapper.py --hash 482c811da5d5b4bc6d497ffa98491e38 --john
python3 hash_wrapper.py --file temp.txt --john
```
...
python3 hash_wrapper.py HASH --auto

# Save only recovered hash/password lines
python3 hash_wrapper.py HASH --john --output cracked.txt

# Named restorable session
python3 hash_wrapper.py HASH --session challenge-01

# Mask or incremental brute force
python3 hash_wrapper.py HASH --hashcat --attack mask --mask '?d?d?d?d' --increment
python3 hash_wrapper.py HASH --john --attack bruteforce

# Read an existing potfile result without cracking again
python3 hash_wrapper.py HASH --john --show
```
...
| `batch_diff.py`       | Strict-majority SHA256/size outliers with optional recursive discovery                    | No                                        |
| `stego_runner.py`     | Native zero-width/appended signatures plus steghide/stegseek/zsteg                        | External extractors may write payloads    |
| `encoding_decoder.py` | Base64/URL, Base32/85, hex, binary, Morse, ROT47, and Atbash decoding                     | No                                        |
...
```bash
python3 triage.py unknown.bin
python3 triage.py unknown.bin --verbose --output triage.txt
python3 triage.py unknown.bin --extract   # explicitly allows binwalk extraction
```
...
```bash
python3 batch_diff.py directory/
python3 batch_diff.py file1.png file2.png file3.png
python3 batch_diff.py directory/ --method size --ext .png .jpg
```
...
```bash
python3 encoding_decoder.py encoded.txt --recursive
python3 encoding_decoder.py --text '666c6167' --decode hex
python3 encoding_decoder.py --file nested.txt --recursive --verbose
```
...
```bash
python3 cipher_solver.py cipher.txt --cipher-type caesar
python3 cipher_solver.py --text 'uryyb' --cipher-type caesar --caesar-shift 13
python3 cipher_solver.py cipher.txt --cipher-type vigenere --key-length 6
python3 cipher_solver.py cipher.txt --cipher-type vigenere --key SECRET
```

### XOR

```bash
python3 xor_toolkit.py encrypted.bin --mode single
python3 xor_toolkit.py encrypted.bin --mode detect
python3 xor_toolkit.py encrypted.bin --mode repeating --key KEY
python3 xor_toolkit.py --hex '00112233' --mode crib --key 'flag{' --key-length 3
```
...
```bash
python3 rsa_solver.py -n 3233 -e 17 -c 2790 -p 61 -q 53
python3 rsa_solver.py -n n.txt -e e.txt -c c.txt -d d.txt
python3 rsa_solver.py -e 3 -c CIPHERTEXT --attack lowe
python3 rsa_solver.py -n MODULUS -e EXPONENT -c CIPHERTEXT --attack fermat
python3 rsa_solver.py -n MODULUS -e EXPONENT --attack wiener
python3 rsa_solver.py -n MODULUS -1 C1 -c C2 --e1 E1 --e2 E2 --attack commonmod
```
...
```bash
python3 dh_solver.py message.txt
python3 dh_solver.py --params message.txt --top 5
python3 dh_solver.py --g 2 --p p.txt --A A.txt --b b.txt --enc-hex ffe6ece0...
python3 dh_solver.py --g 2 --p 23 --a 6 --B 19 --enc-hex '...' --output dh-report.txt
```
...
```bash
python3 stego_runner.py image.jpg --passphrase secret --extract-to payload.bin
python3 stego_runner.py image.jpg --wordlist words.txt --extract-to payload.bin
python3 stego_runner.py image.png --tool zsteg --output zsteg-report.txt
python3 stego_runner.py image.jpg --extract-to payload.bin --force -v
```
...
```bash
python3 archive_cracker.py challenge.zip --wordlist words.txt
python3 archive_cracker.py challenge.zip --extract --output-dir extracted/
python3 archive_cracker.py challenge.zip --auto --output-dir extracted/
```
...
```bash
python3 pcap_forensics.py capture.pcap
python3 pcap_forensics.py capture.pcap --stream 0
python3 pcap_forensics.py capture.pcap -x --directory http_objects/
```
...
```bash
python3 audio_forensics.py audio.wav
python3 audio_forensics.py audio.wav --spectrogram spectrogram.png
python3 audio_forensics.py audio.wav --wavsteg --wavsteg-output payload.bin
python3 audio_forensics.py audio.mp3 --id3
