#!/usr/bin/env python3
"""Read-only offline PostgreSQL plain-SQL dump inspection (no live connections)."""
...
def extract_balanced(text: str, start: int) -> tuple[str, int] | None:
    """Return (inner, index_after_close) for the paren opened at start."""
...
def parse_create_tables(sql: str) -> dict[str, dict[str, object]]:
    ...
    pattern = re.compile(
        r"CREATE\s+(?:TEMPORARY\s+|TEMP\s+|UNLOGGED\s+)?TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?(?P<name>[^\s(]+)",
        re.IGNORECASE,
    )
    for match in pattern.finditer(sql):
        raw_name = match.group("name").strip().rstrip(";")
        # Skip preceding comment noise: name must not start with - or /.
        ...
        if paren_start == -1:
            continue
        # Ensure the paren belongs to this statement (no semicolon in between).
        if ";" in sql[match.end() : paren_start].replace("\n", " "):
            # Allow newlines but not a statement terminator.
            segment = sql[match.end() : paren_start]
            if ";" in segment:
                continue
        ...
        for part in split_top_level(inner):
            ...
            upper = token.upper()
            if upper.startswith(
                ("CONSTRAINT", "PRIMARY", "FOREIGN", "UNIQUE", "CHECK", "EXCLUDE", "LIKE ", "KEY ", "INDEX")
            ):
                continue
...
def parse_insert_values(values_text: str) -> list[list[str | None]]:
    ...
    while index < len(values_text):
        while index < len(values_text) and values_text[index] not in "(,":
            # Skip commas/whitespace between tuples; stop at semicolon.
...
def parse_inserts(sql: str, tables: dict[str, dict[str, object]]) -> None:
    pattern = re.compile(
        r"INSERT\s+INTO\s+(?P<name>[^\s(]+)(?:\s*\((?P<cols>[^)]+)\))?\s*VALUES\s*(?P<values>.*?);",
    ...
    for match in pattern.finditer(sql):
        ...
        for values in parse_insert_values(match.group("values")):
            ...
            if len(entry["rows"]) < 10_000:  # bound memory; report caps display later
...
def parse_copies(lines: list[str], tables: dict[str, dict[str, object]]) -> list[str]:
    warnings: list[str] = []
    copy_re = re.compile(
        r"^COPY\s+(?P<name>[^\s(]+)(?:\s*\((?P<cols>[^)]+)\))?\s+FROM\s+stdin\s*;",
        re.IGNORECASE,
    )
    index = 0
    while index < len(lines):
        match = copy_re.match(lines[index].strip())
        if not match:
            index += 1
            continue
        raw_name = match.group("name").strip()
        ...
        while index < len(lines):
            ...
            if line == r"\.":
                terminated = True
                break
            fields: list[str | None] = [None if field == r"\N" else field for field in line.split("\t")]
            entry["count"] += 1  # type: ignore[typeddict-item]
            if len(entry["rows"]) < 10_000:
                if columns and len(fields) == len(columns):
                    entry["rows"].append(dict(zip(columns, fields)))  # type: ignore[arg-type]
                else:
                    entry["rows"].append({f"col{i+1}": value for i, value in enumerate(fields)})
        if not terminated:
            warnings.append(f"COPY block for '{name}' is missing its \\. terminator")
    return warnings
...
def analyze_dump(path: Path, row_limit: int, include_strings: bool) -> dict[str, object]:
    ...
    if raw[:5] == b"PGDMP":
        raise DumpError("custom-format pg_dump detected (PGDMP magic); convert first: pg_restore -f dump.sql dump.pgdump")
    if b"\x00" in raw[:8192] and len(raw) > 0:
        # Plain SQL dumps are text; NUL bytes imply custom/tar/binary input.
    ...
    if any(marker in text for marker in PG_MARKERS):
        ...
        warnings.append("no PostgreSQL markers found; showing generic plain-SQL parse")

    # Strip /* */ block comments for parsing (keep line structure).
    scrubbed = re.sub(r"/\*.*?\*/", "", text, flags=re.DOTALL)
    lines = scrubbed.splitlines()
    # Skip fully commented lines for COPY scanning.
    ...
    warnings.extend(parse_copies(code_lines, tables))

    total_rows = sum(int(entry["count"]) for entry in tables.values())
    if not tables:
        warnings.append("no CREATE TABLE / INSERT / COPY statements parsed")
    return {
        "path": str(path),
        "flavor": flavor,
        "size": len(raw),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Inspect offline PostgreSQL plain-SQL dumps without a live server",
        allow_abbrev=False,
    )
    parser.add_argument("dump", help="Plain-SQL dump file (.sql)")
